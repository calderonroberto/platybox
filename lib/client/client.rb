...
module Platybox
  class Client
    ...
    #returns the profile of a place, need not be authenticated
      def places_profile (bits_id)
        url = URI.parse( @site + "/1/places/profile")
        res = Net::HTTP::post_form(url, {:bits_id => bits_id})
        @response = res
        @place = JSON.parse(@response.body())        
      end
